...
var _ = SIGDescribe("Hotplug", func() {
	...
	Context("[storage-req]", decorators.StorageReq, func() {
		...
		Context("Online VM with ejected and inserted cdroms attached", func() {
			...
			FDescribeTable("should eject / insert cdrom", func(addVolumeFunc addVolumeFunction, removeVolumeFunc removeVolumeFunction, vmiOnly, waitToStart bool) {
		...
		Context("Online VM with many ejected cdroms attached", func() {
			...
			BeforeEach(func() {
				...
				addEjectedAndInsertedVolumesAndDisksOnSpec(vm, "", "volume3", "", "")
				addEjectedAndInsertedVolumesAndDisksOnSpec(vm, "", "volume4", "", "")
			...
			FDescribeTable("Should be able to insert and eject multiple cdrom volumes", func(addVolumeFunc addVolumeFunction, removeVolumeFunc removeVolumeFunction, vmiOnly bool) {
				vmi, err := virtClient.VirtualMachineInstance(vm.Namespace).Get(context.Background(), vm.Name, &metav1.GetOptions{})
				Expect(err).ToNot(HaveOccurred())
				getVmiConsoleAndLogin(vmi)
				libwait.WaitForSuccessfulVMIStartWithTimeout(vmi, 240)
