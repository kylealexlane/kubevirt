...
func (l *LibvirtDomainManager) SyncVMI(vmi *v1.VirtualMachineInstance, allowEmulation bool, options *cmdv1.VirtualMachineOptions) (*api.DomainSpec, error) {
	...
	for _, diskToEject := range newlyEjectedCDRoms {
		...
		err = dom.UpdateDeviceFlags(strings.ToLower(string(diskBytes)), affectLiveAndConfigLibvirtFlags)
		if err != nil {
	...
	for _, diskToInsert := range newlyInsertedCDRoms {
		...
		err = dom.UpdateDeviceFlags(strings.ToLower(string(bytes)), affectLiveAndConfigLibvirtFlags)
		if err != nil {
